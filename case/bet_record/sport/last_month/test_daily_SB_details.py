...
from case.bet_record.sport.last_month import get_last_month_list



data = ExcelUtil("betrecord").dict_data()
class BetRecordSB(unittest.TestCase):
	'''测试获取上月每天的沙巴体育投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_SB_bet_details_daily(self):

		route = data[208]["route"]
		url = "".join(base.get_url(route))
		Method = data[208]["method"]
		params = eval(data[208]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[208]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[208]["expect"],resp.json()["message"]))
