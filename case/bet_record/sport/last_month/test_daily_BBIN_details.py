...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		'''测试获取上月每天的BBIN体育投注详情'''
		route = data[211]["route"]
		url = "".join(base.get_url(route))
		Method = data[211]["method"]
		params = eval(data[211]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[211]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[211]["expect"],resp.json()["message"]))
