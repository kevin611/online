...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[207]["route"]
		url = "".join(base.get_url(route))
		Method = data[207]["method"]
		params = eval(data[207]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[207]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[207]["expect"],resp.json()["message"]))
