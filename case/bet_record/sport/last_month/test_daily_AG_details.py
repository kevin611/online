...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		...
		route = data[210]["route"]
		url = "".join(base.get_url(route))
		Method = data[210]["method"]
		params = eval(data[210]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[210]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[210]["expect"],resp.json()["message"]))
