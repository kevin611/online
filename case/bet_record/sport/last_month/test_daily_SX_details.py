...
class BetRecordSX(unittest.TestCase):
	...
	def test_SX_bet_details_daily(self):
		'''测试获取上月每天的皇冠体育投注详情'''
		route = data[209]["route"]
		url = "".join(base.get_url(route))
		Method = data[209]["method"]
		params = eval(data[209]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[209]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[209]["expect"],resp.json()["message"]))
