...
class BetRecordSB(unittest.TestCase):
	...
	def test_SB_bet_details_daily(self):
		'''测试获取本周每天的沙巴体育投注详情'''
		route = data[190]["route"]
		url = "".join(base.get_url(route))
		Method = data[190]["method"]
		params = eval(data[190]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[190]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[190]["expect"],resp.json()["message"]))
