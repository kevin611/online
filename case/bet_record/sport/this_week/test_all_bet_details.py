...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		'''测试获取本周每天的全部投注详情'''
		route = data[189]["route"]
		url = "".join(base.get_url(route))
		Method = data[189]["method"]
		params = eval(data[189]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[189]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[189]["expect"],resp.json()["message"]))
