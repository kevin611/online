...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		...
		route = data[193]["route"]
		url = "".join(base.get_url(route))
		Method = data[193]["method"]
		params = eval(data[193]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[193]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[193]["expect"],resp.json()["message"]))
