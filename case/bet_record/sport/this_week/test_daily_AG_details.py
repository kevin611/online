...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		...
		route = data[192]["route"]
		url = "".join(base.get_url(route))
		Method = data[192]["method"]
		params = eval(data[192]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[192]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[192]["expect"],resp.json()["message"]))
