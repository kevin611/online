...
class BetRecordSX(unittest.TestCase):
	...
	def test_SX_bet_details_daily(self):
		...
		route = data[191]["route"]
		url = "".join(base.get_url(route))
		Method = data[191]["method"]
		params = eval(data[191]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[191]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[191]["expect"],resp.json()["message"]))
