...
class BetRecordSB(unittest.TestCase):
	...
	def test_SB_bet_details_daily(self):
		...
		route = data[196]["route"]
		url = "".join(base.get_url(route))
		Method = data[196]["method"]
		params = eval(data[196]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[196]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[196]["expect"],resp.json()["message"]))
