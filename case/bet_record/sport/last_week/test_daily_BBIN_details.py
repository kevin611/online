...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		'''测试获取上周每天的BBIN体育投注详情'''
		route = data[199]["route"]
		url = "".join(base.get_url(route))
		Method = data[199]["method"]
		params = eval(data[199]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[199]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[199]["expect"],resp.json()["message"]))
