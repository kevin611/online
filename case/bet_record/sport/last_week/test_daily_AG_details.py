...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		'''测试获取上周每天的AG体育投注详情'''
		route = data[198]["route"]
		url = "".join(base.get_url(route))
		Method = data[198]["method"]
		params = eval(data[198]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[198]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[198]["expect"],resp.json()["message"]))
