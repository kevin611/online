...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		'''测试获取上周每天的全部投注详情'''
		route = data[195]["route"]
		url = "".join(base.get_url(route))
		Method = data[195]["method"]
		params = eval(data[195]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[195]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[195]["expect"],resp.json()["message"]))
