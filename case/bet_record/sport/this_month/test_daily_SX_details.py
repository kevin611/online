...
class BetRecordSX(unittest.TestCase):
	...
	def test_SX_bet_details_daily(self):
		'''测试获取本月每天的皇冠体育投注详情'''
		route = data[203]["route"]
		url = "".join(base.get_url(route))
		Method = data[203]["method"]
		params = eval(data[203]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[203]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[203]["expect"],resp.json()["message"]))
