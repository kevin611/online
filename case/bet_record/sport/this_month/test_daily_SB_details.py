...
class BetRecordSB(unittest.TestCase):
	...
	def test_SB_bet_details_daily(self):
		...
		route = data[202]["route"]
		url = "".join(base.get_url(route))
		Method = data[202]["method"]
		params = eval(data[202]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[202]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[202]["expect"],resp.json()["message"]))
