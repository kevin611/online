...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[201]["route"]
		url = "".join(base.get_url(route))
		Method = data[201]["method"]
		params = eval(data[201]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[201]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[201]["expect"],resp.json()["message"]))
