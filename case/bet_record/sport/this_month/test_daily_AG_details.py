...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		'''测试获取本月每天的AG体育投注详情'''
		route = data[204]["route"]
		url = "".join(base.get_url(route))
		Method = data[204]["method"]
		params = eval(data[204]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[204]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[204]["expect"],resp.json()["message"]))
