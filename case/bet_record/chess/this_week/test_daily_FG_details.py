...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[218]["route"]
		url = "".join(base.get_url(route))
		Method = data[218]["method"]
		params = eval(data[218]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[218]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[218]["expect"],resp.json()["message"]))
