...
class BetRecordLEG(unittest.TestCase):
	...
	def test_LEG_bet_details_daily(self):
		'''测试获取本周每天的乐游棋牌投注详情'''
		route = data[215]["route"]
		url = "".join(base.get_url(route))
		Method = data[215]["method"]
		params = eval(data[215]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[215]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[215]["expect"],resp.json()["message"]))
