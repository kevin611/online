...
class BetRecordVG(unittest.TestCase):
	...
	def test_VG_bet_details_daily(self):
		'''测试获取本周每天的VG棋牌投注详情'''
		route = data[217]["route"]
		url = "".join(base.get_url(route))
		Method = data[217]["method"]
		params = eval(data[217]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[217]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[217]["expect"],resp.json()["message"]))
