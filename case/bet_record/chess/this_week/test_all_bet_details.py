...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[213]["route"]
		url = "".join(base.get_url(route))
		Method = data[213]["method"]
		params = eval(data[213]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[213]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[213]["expect"],resp.json()["message"]))
