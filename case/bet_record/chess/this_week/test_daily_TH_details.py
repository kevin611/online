...
class BetRecordTH(unittest.TestCase):
	...
	def test_TH_bet_details_daily(self):
		'''测试获取本周每天的天豪棋牌投注详情'''
		route = data[214]["route"]
		url = "".join(base.get_url(route))
		Method = data[214]["method"]
		params = eval(data[214]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[214]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[214]["expect"],resp.json()["message"]))
