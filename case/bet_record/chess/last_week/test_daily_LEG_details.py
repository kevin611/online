...
class BetRecordLEG(unittest.TestCase):
	...
	def test_LEG_bet_details_daily(self):
		...
		route = data[222]["route"]
		url = "".join(base.get_url(route))
		Method = data[222]["method"]
		params = eval(data[222]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[222]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[222]["expect"],resp.json()["message"]))
