...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[225]["route"]
		url = "".join(base.get_url(route))
		Method = data[225]["method"]
		params = eval(data[225]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[225]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[225]["expect"],resp.json()["message"]))
