...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[220]["route"]
		url = "".join(base.get_url(route))
		Method = data[220]["method"]
		params = eval(data[220]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[220]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[220]["expect"],resp.json()["message"]))
