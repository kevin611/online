...
class BetRecordKY(unittest.TestCase):
	...
	def test_KY_bet_details_daily(self):
		'''测试获取上周每天的开元棋牌投注详情'''
		route = data[223]["route"]
		url = "".join(base.get_url(route))
		Method = data[223]["method"]
		params = eval(data[223]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[223]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[223]["expect"],resp.json()["message"]))
