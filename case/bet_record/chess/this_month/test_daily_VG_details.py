...
class BetRecordVG(unittest.TestCase):
	...
	def test_VG_bet_details_daily(self):
		...
		route = data[231]["route"]
		url = "".join(base.get_url(route))
		Method = data[231]["method"]
		params = eval(data[231]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[231]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[231]["expect"],resp.json()["message"]))
