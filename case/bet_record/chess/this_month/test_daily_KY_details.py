...
class BetRecordKY(unittest.TestCase):
	...
	def test_KY_bet_details_daily(self):
		'''测试获取本月每天的开元棋牌投注详情'''
		route = data[230]["route"]
		url = "".join(base.get_url(route))
		Method = data[230]["method"]
		params = eval(data[230]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[230]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[230]["expect"],resp.json()["message"]))
