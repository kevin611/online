...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[227]["route"]
		url = "".join(base.get_url(route))
		Method = data[227]["method"]
		params = eval(data[227]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[227]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[227]["expect"],resp.json()["message"]))
