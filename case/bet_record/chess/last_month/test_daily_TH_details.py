...
class BetRecordTH(unittest.TestCase):
	...
	def test_TH_bet_details_daily(self):
		'''测试获取上月每天的天豪棋牌投注详情'''
		route = data[235]["route"]
		url = "".join(base.get_url(route))
		Method = data[235]["method"]
		params = eval(data[235]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[235]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[235]["expect"],resp.json()["message"]))
