...
class BetRecordLEG(unittest.TestCase):
	...
	def test_LEG_bet_details_daily(self):
		'''测试获取上月每天的乐游棋牌投注详情'''
		route = data[236]["route"]
		url = "".join(base.get_url(route))
		Method = data[236]["method"]
		params = eval(data[236]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[236]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[236]["expect"],resp.json()["message"]))
