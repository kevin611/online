...
class BetRecordKY(unittest.TestCase):
	...
	def test_KY_bet_details_daily(self):
		...
		route = data[237]["route"]
		url = "".join(base.get_url(route))
		Method = data[237]["method"]
		params = eval(data[237]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[237]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[237]["expect"],resp.json()["message"]))
