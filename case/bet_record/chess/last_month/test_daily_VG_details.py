...
from case.bet_record.chess.last_month import get_last_month_list



data = ExcelUtil("betrecord").dict_data()
class BetRecordVG(unittest.TestCase):
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_VG_bet_details_daily(self):
		'''测试获取上月每天的VG棋牌投注详情'''
		route = data[238]["route"]
		url = "".join(base.get_url(route))
		Method = data[238]["method"]
		params = eval(data[238]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[238]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[238]["expect"],resp.json()["message"]))
