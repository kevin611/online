...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[239]["route"]
		url = "".join(base.get_url(route))
		Method = data[239]["method"]
		params = eval(data[239]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[239]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[239]["expect"],resp.json()["message"]))
