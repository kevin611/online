...
class BetRecordDG(unittest.TestCase):
	...
	def test_DG_bet_details_daily(self):
		...
		route = data[115]["route"]
		url = "".join(base.get_url(route))
		Method = data[115]["method"]
		params = eval(data[115]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[115]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[115]["expect"],resp.json()["message"]))
