...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		'''测试获取本月每天的BBIN投注详情'''
		route = data[113]["route"]
		url = "".join(base.get_url(route))
		Method = data[113]["method"]
		params = eval(data[113]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[113]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[113]["expect"],resp.json()["message"]))
