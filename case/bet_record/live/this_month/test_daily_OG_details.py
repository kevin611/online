...
class BetRecordOG(unittest.TestCase):
	...
	def test_OG_bet_details_daily(self):
		...
		route = data[118]["route"]
		url = "".join(base.get_url(route))
		Method = data[118]["method"]
		params = eval(data[118]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[118]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[118]["expect"],resp.json()["message"]))
