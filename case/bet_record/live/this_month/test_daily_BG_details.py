...
class BetRecordBG(unittest.TestCase):
	...
	def test_BG_bet_details_daily(self):
		...
		route = data[114]["route"]
		url = "".join(base.get_url(route))
		Method = data[114]["method"]
		params = eval(data[114]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[114]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[114]["expect"],resp.json()["message"]))
