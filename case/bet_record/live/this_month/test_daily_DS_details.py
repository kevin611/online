...
class BetRecord115(unittest.TestCase):
	...
	def test_DS_bet_details_daily(self):
		'''测试获取本月每天的DS投注详情'''
		route = data[116]["route"]
		url = "".join(base.get_url(route))
		Method = data[116]["method"]
		params = eval(data[116]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[116]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[116]["expect"],resp.json()["message"]))
