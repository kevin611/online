...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		'''测试获取本月每天的全部投注详情'''
		route = data[111]["route"]
		url = "".join(base.get_url(route))
		Method = data[111]["method"]
		params = eval(data[111]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[111]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[111]["expect"],resp.json()["message"]))
