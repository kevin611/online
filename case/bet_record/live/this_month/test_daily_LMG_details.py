...
class BetRecordLMG(unittest.TestCase):
	'''测试获取本月每天的LMG投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_LMG_bet_details_daily(self):

		route = data[117]["route"]
		url = "".join(base.get_url(route))
		Method = data[117]["method"]
		params = eval(data[117]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[117]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[117]["expect"],resp.json()["message"]))
