...
class BetRecord115(unittest.TestCase):
	...
	def test_SA_bet_details_daily(self):
		'''测试获取本月每天的SA投注详情'''
		route = data[120]["route"]
		url = "".join(base.get_url(route))
		Method = data[120]["method"]
		params = eval(data[120]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[120]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[120]["expect"],resp.json()["message"]))
