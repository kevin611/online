...
class BetRecord115(unittest.TestCase):
	...
	def test_AB_bet_details_daily(self):
		'''测试获取本月每天的AB投注详情'''
		route = data[119]["route"]
		url = "".join(base.get_url(route))
		Method = data[119]["method"]
		params = eval(data[119]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[119]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[119]["expect"],resp.json()["message"]))
