...
class BetRecordEBET(unittest.TestCase):
	...
	def test_EBET_bet_details_daily(self):
		'''测试获取本月每天的EBET投注详情'''
		route = data[122]["route"]
		url = "".join(base.get_url(route))
		Method = data[122]["method"]
		params = eval(data[122]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[122]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[122]["expect"],resp.json()["message"]))
