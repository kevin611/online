...
class BetRecordLEBO(unittest.TestCase):
	'''测试获取本月每天的LEBO投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_LEBO_bet_details_daily(self):

		route = data[121]["route"]
		url = "".join(base.get_url(route))
		Method = data[121]["method"]
		params = eval(data[121]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[121]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[121]["expect"],resp.json()["message"]))
