...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		'''测试获取本月每天的AG投注详情'''
		route = data[112]["route"]
		url = "".join(base.get_url(route))
		Method = data[112]["method"]
		params = eval(data[112]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[112]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[112]["expect"],resp.json()["message"]))
