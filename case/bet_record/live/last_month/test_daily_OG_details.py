...
class BetRecordOG(unittest.TestCase):
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_OG_bet_details_daily(self):
		'''测试获取上月每天的OG投注详情'''
		route = data[131]["route"]
		url = "".join(base.get_url(route))
		Method = data[131]["method"]
		params = eval(data[131]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[131]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[131]["expect"],resp.json()["message"]))
