...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		...
		route = data[125]["route"]
		url = "".join(base.get_url(route))
		Method = data[125]["method"]
		params = eval(data[125]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[125]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[125]["expect"],resp.json()["message"]))
