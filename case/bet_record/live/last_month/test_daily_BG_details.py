...
class BetRecordBG(unittest.TestCase):
	...
	def test_BG_bet_details_daily(self):
		...
		route = data[127]["route"]
		url = "".join(base.get_url(route))
		Method = data[127]["method"]
		params = eval(data[127]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[127]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[127]["expect"],resp.json()["message"]))
