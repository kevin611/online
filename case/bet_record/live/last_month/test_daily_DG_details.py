...
class BetRecordDG(unittest.TestCase):
	'''测试获取上月每天的DG投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_DG_bet_details_daily(self):

		route = data[128]["route"]
		url = "".join(base.get_url(route))
		Method = data[128]["method"]
		params = eval(data[128]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[128]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[128]["expect"],resp.json()["message"]))
