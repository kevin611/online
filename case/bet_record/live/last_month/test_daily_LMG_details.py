...
class BetRecordLMG(unittest.TestCase):
	...
	def test_LMG_bet_details_daily(self):
		...
		route = data[130]["route"]
		url = "".join(base.get_url(route))
		Method = data[130]["method"]
		params = eval(data[130]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[130]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[130]["expect"],resp.json()["message"]))
