...
class BetRecordSA(unittest.TestCase):
	...
	def test_SA_bet_details_daily(self):
		'''测试获取上月每天的SA投注详情'''
		route = data[133]["route"]
		url = "".join(base.get_url(route))
		Method = data[133]["method"]
		params = eval(data[133]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[133]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[133]["expect"],resp.json()["message"]))
