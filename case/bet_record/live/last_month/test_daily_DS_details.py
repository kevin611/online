...
class BetRecordDS(unittest.TestCase):
	...
	def test_DS_bet_details_daily(self):
		...
		route = data[129]["route"]
		url = "".join(base.get_url(route))
		Method = data[129]["method"]
		params = eval(data[129]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[129]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[129]["expect"],resp.json()["message"]))
