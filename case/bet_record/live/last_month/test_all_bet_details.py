...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		'''测试获取上月每天的全部投注详情'''
		route = data[124]["route"]
		url = "".join(base.get_url(route))
		Method = data[124]["method"]
		params = eval(data[124]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[124]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[124]["expect"],resp.json()["message"]))
