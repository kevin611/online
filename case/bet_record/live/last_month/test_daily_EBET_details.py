...
class BetRecordEBET(unittest.TestCase):
	...
	def test_EBET_bet_details_daily(self):
		'''测试获取上月每天的EBET投注详情'''
		route = data[135]["route"]
		url = "".join(base.get_url(route))
		Method = data[135]["method"]
		params = eval(data[135]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[135]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[135]["expect"],resp.json()["message"]))
