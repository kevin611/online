...
class BetRecordLEBO(unittest.TestCase):
	...
	def test_LEBO_bet_details_daily(self):
		'''测试获取上月每天的LEBO投注详情'''
		route = data[134]["route"]
		url = "".join(base.get_url(route))
		Method = data[134]["method"]
		params = eval(data[134]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[134]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[134]["expect"],resp.json()["message"]))
