...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		...
		route = data[126]["route"]
		url = "".join(base.get_url(route))
		Method = data[126]["method"]
		params = eval(data[126]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[126]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[126]["expect"],resp.json()["message"]))
