...
class BetRecordLMG(unittest.TestCase):
	...
	def test_LMG_bet_details_daily(self):
		'''测试获取上周每天的LMG投注详情'''
		route = data[104]["route"]
		url = "".join(base.get_url(route))
		Method = data[104]["method"]
		params = eval(data[104]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[104]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[104]["expect"],resp.json()["message"]))
