...
class BetRecordDG(unittest.TestCase):
	...
	def test_DG_bet_details_daily(self):
		...
		route = data[102]["route"]
		url = "".join(base.get_url(route))
		Method = data[102]["method"]
		params = eval(data[102]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[102]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[102]["expect"],resp.json()["message"]))
