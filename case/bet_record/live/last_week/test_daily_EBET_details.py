...
class BetRecordEBET(unittest.TestCase):
	'''测试获取上周每天的EBET投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_EBET_bet_details_daily(self):

		route = data[109]["route"]
		url = "".join(base.get_url(route))
		Method = data[109]["method"]
		params = eval(data[109]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[109]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[109]["expect"],resp.json()["message"]))
