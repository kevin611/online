...
class BetRecordSA(unittest.TestCase):
	...
	def test_SA_bet_details_daily(self):
		...
		route = data[107]["route"]
		url = "".join(base.get_url(route))
		Method = data[107]["method"]
		params = eval(data[107]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[107]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[107]["expect"],resp.json()["message"]))
