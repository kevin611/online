...
class BetRecordLEBO(unittest.TestCase):
	...
	def test_LEBO_bet_details_daily(self):
		...
		route = data[108]["route"]
		url = "".join(base.get_url(route))
		Method = data[108]["method"]
		params = eval(data[108]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[108]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[108]["expect"],resp.json()["message"]))
