...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		'''测试获取上周每天的BBIN投注详情'''
		route = data[100]["route"]
		url = "".join(base.get_url(route))
		Method = data[100]["method"]
		params = eval(data[100]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[100]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[100]["expect"],resp.json()["message"]))
