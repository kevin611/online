...
class BetRecord115(unittest.TestCase):
	...
	def test_AB_bet_details_daily(self):
		'''测试获取上周每天的AB投注详情'''
		route = data[106]["route"]
		url = "".join(base.get_url(route))
		Method = data[106]["method"]
		params = eval(data[106]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[106]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[106]["expect"],resp.json()["message"]))
