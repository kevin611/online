...
class BetRecordOG(unittest.TestCase):
	...
	def test_OG_bet_details_daily(self):
		...
		route = data[105]["route"]
		url = "".join(base.get_url(route))
		Method = data[105]["method"]
		params = eval(data[105]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[105]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[105]["expect"],resp.json()["message"]))
