...
class BetRecordPT(unittest.TestCase):
	...
	def test_PT_bet_details_daily(self):
		...
		route = data[148]["route"]
		url = "".join(base.get_url(route))
		Method = data[148]["method"]
		params = eval(data[148]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[148]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[148]["expect"],resp.json()["message"]))
