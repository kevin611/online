...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[137]["route"]
		url = "".join(base.get_url(route))
		Method = data[137]["method"]
		params = eval(data[137]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[137]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[137]["expect"],resp.json()["message"]))
