...
class BetRecordMW(unittest.TestCase):
	...
	def test_MW_bet_details_daily(self):
		...
		route = data[144]["route"]
		url = "".join(base.get_url(route))
		Method = data[144]["method"]
		params = eval(data[144]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[144]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[144]["expect"],resp.json()["message"]))
