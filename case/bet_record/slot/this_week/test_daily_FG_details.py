...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[139]["route"]
		url = "".join(base.get_url(route))
		Method = data[139]["method"]
		params = eval(data[139]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[139]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[139]["expect"],resp.json()["message"]))
