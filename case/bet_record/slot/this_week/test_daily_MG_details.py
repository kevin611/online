...
class BetRecordMG(unittest.TestCase):
	...
	def test_MG_bet_details_daily(self):
		'''测试获取本周每天的MG电子投注详情'''
		route = data[140]["route"]
		url = "".join(base.get_url(route))
		Method = data[140]["method"]
		params = eval(data[140]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[140]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[140]["expect"],resp.json()["message"]))
