...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		...
		route = data[138]["route"]
		url = "".join(base.get_url(route))
		Method = data[138]["method"]
		params = eval(data[138]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[138]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[138]["expect"],resp.json()["message"]))
