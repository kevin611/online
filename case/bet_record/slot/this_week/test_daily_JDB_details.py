...
class BetRecordJDB(unittest.TestCase):
	...
	def test_JDB_bet_details_daily(self):
		'''测试获取本周每天的JDB电子投注详情'''
		route = data[142]["route"]
		url = "".join(base.get_url(route))
		Method = data[142]["method"]
		params = eval(data[142]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[142]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[142]["expect"],resp.json()["message"]))
