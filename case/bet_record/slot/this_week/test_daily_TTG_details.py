...
class BetRecordTTG(unittest.TestCase):
	...
	def test_TTG_bet_details_daily(self):
		...
		route = data[147]["route"]
		url = "".join(base.get_url(route))
		Method = data[147]["method"]
		params = eval(data[147]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[147]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[147]["expect"],resp.json()["message"]))
