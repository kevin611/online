...
class BetRecordAE(unittest.TestCase):
	...
	def test_AE_bet_details_daily(self):
		'''测试获取本周每天的AE电子投注详情'''
		route = data[146]["route"]
		url = "".join(base.get_url(route))
		Method = data[146]["method"]
		params = eval(data[146]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[146]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[146]["expect"],resp.json()["message"]))
