...
class BetRecordDT(unittest.TestCase):
	...
	def test_DT_bet_details_daily(self):
		'''测试获取上周每天的DT电子投注详情'''
		route = data[158]["route"]
		url = "".join(base.get_url(route))
		Method = data[158]["method"]
		params = eval(data[158]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[158]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[158]["expect"],resp.json()["message"]))
