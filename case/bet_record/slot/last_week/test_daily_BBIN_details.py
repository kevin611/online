...
class BetRecordBBBIN(unittest.TestCase):
	'''测试获取上周每天的BBIN电子投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_BBIN_bet_details_daily(self):

		route = data[156]["route"]
		url = "".join(base.get_url(route))
		Method = data[156]["method"]
		params = eval(data[156]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[156]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[156]["expect"],resp.json()["message"]))
