...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[152]["route"]
		url = "".join(base.get_url(route))
		Method = data[152]["method"]
		params = eval(data[152]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[152]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[152]["expect"],resp.json()["message"]))
