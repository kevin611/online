...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[150]["route"]
		url = "".join(base.get_url(route))
		Method = data[150]["method"]
		params = eval(data[150]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[150]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[150]["expect"],resp.json()["message"]))
