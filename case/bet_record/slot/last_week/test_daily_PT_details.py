...
class BetRecordPT(unittest.TestCase):
	...
	def test_PT_bet_details_daily(self):
		'''测试获取上周每天的PT电子投注详情'''
		route = data[161]["route"]
		url = "".join(base.get_url(route))
		Method = data[161]["method"]
		params = eval(data[161]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[161]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[161]["expect"],resp.json()["message"]))
