...
class BetRecordMW(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		...
		route = data[157]["route"]
		url = "".join(base.get_url(route))
		Method = data[157]["method"]
		params = eval(data[157]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[157]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[157]["expect"],resp.json()["message"]))
