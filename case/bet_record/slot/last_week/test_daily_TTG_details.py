...
class BetRecordTTG(unittest.TestCase):
	...
	def test_TTG_bet_details_daily(self):
		...
		route = data[160]["route"]
		url = "".join(base.get_url(route))
		Method = data[160]["method"]
		params = eval(data[160]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[160]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[160]["expect"],resp.json()["message"]))
