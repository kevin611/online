...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		...
		route = data[151]["route"]
		url = "".join(base.get_url(route))
		Method = data[151]["method"]
		params = eval(data[151]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[151]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[151]["expect"],resp.json()["message"]))
