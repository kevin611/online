...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[163]["route"]
		url = "".join(base.get_url(route))
		Method = data[163]["method"]
		params = eval(data[163]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[163]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[163]["expect"],resp.json()["message"]))
