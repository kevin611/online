...
class BetRecordAE(unittest.TestCase):
	...
	def test_AE_bet_details_daily(self):
		...
		route = data[172]["route"]
		url = "".join(base.get_url(route))
		Method = data[172]["method"]
		params = eval(data[172]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[172]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[172]["expect"],resp.json()["message"]))
