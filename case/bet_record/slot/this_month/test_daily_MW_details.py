...
class BetRecordMW(unittest.TestCase):
	'''测试获取本月每天的MW电子投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_MW_bet_details_daily(self):

		route = data[170]["route"]
		url = "".join(base.get_url(route))
		Method = data[170]["method"]
		params = eval(data[170]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[170]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[170]["expect"],resp.json()["message"]))
