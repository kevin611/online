...
class BetRecordBBIN(unittest.TestCase):
	...
	def test_BBIN_bet_details_daily(self):
		...
		route = data[169]["route"]
		url = "".join(base.get_url(route))
		Method = data[169]["method"]
		params = eval(data[169]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[169]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[169]["expect"],resp.json()["message"]))
