...
class BetRecordList(unittest.TestCase):
	...
	def test_get_slotlist_this_week(self):
		res = get_this_month_list.BetRecord().get_betrecord_list() # 本月电子投注记录
		self.log.info("----------test is start----------")
		self.log.info("请求的接口地址为 %s" %res.url)
		self.log.info("响应内容为: %s" % res.json())
		self.log.info("响应状态码为: %s" % res.status_code)
		self.assertEqual(data[162]["expect"],res.json()["message"],msg="失败原因%s ！= %s"%(data[162]["expect"],res.json()["message"]))
