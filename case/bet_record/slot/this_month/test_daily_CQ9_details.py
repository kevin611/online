...
class BetRecordCQ9(unittest.TestCase):
	...
	def test_CQ9_bet_details_daily(self):
		...
		route = data[167]["route"]
		url = "".join(base.get_url(route))
		Method = data[167]["method"]
		params = eval(data[167]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[167]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[167]["expect"],resp.json()["message"]))
