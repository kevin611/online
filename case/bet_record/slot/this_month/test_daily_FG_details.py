...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[165]["route"]
		url = "".join(base.get_url(route))
		Method = data[165]["method"]
		params = eval(data[165]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[165]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[165]["expect"],resp.json()["message"]))
