...
class BetRecordDT(unittest.TestCase):
	...
	def test_DT_bet_details_daily(self):
		...
		route = data[171]["route"]
		url = "".join(base.get_url(route))
		Method = data[171]["method"]
		params = eval(data[171]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[171]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[171]["expect"],resp.json()["message"]))
