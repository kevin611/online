...
class BetRecordMG(unittest.TestCase):
	...
	def test_MG_bet_details_daily(self):
		...
		route = data[166]["route"]
		url = "".join(base.get_url(route))
		Method = data[166]["method"]
		params = eval(data[166]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[166]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[166]["expect"],resp.json()["message"]))
