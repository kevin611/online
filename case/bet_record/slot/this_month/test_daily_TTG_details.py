...
class BetRecordTTG(unittest.TestCase):
	'''测试获取本月每天的TTG电子投注详情'''
	def setUp(self):
		# self.token = get_token.GetRes().get_res()
		self.log = logger.Log()
	def test_TTG_bet_details_daily(self):

		route = data[173]["route"]
		url = "".join(base.get_url(route))
		Method = data[173]["method"]
		params = eval(data[173]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[173]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[173]["expect"],resp.json()["message"]))
