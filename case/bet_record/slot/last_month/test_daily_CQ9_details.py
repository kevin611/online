...
class BetRecordCQ9(unittest.TestCase):
	...
	def test_CQ9_bet_details_daily(self):
		...
		route = data[180]["route"]
		url = "".join(base.get_url(route))
		Method = data[180]["method"]
		params = eval(data[180]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[180]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[180]["expect"],resp.json()["message"]))
