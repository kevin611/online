...
class BetRecordDT(unittest.TestCase):
	...
	def test_DT_bet_details_daily(self):
		...
		route = data[184]["route"]
		url = "".join(base.get_url(route))
		Method = data[184]["method"]
		params = eval(data[184]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[184]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[184]["expect"],resp.json()["message"]))
