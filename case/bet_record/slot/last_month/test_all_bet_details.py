...
class BetRecord(unittest.TestCase):
	...
	def test_all_bet_details(self):
		...
		route = data[176]["route"]
		url = "".join(base.get_url(route))
		Method = data[176]["method"]
		params = eval(data[176]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[176]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[176]["expect"],resp.json()["message"]))
