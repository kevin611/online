...
class BetRecordTTG(unittest.TestCase):
	...
	def test_TTG_bet_details_daily(self):
		...
		route = data[186]["route"]
		url = "".join(base.get_url(route))
		Method = data[186]["method"]
		params = eval(data[186]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[186]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[186]["expect"],resp.json()["message"]))
