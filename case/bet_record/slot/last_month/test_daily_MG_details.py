...
class BetRecordMG(unittest.TestCase):
	...
	def test_MG_bet_details_daily(self):
		...
		route = data[179]["route"]
		url = "".join(base.get_url(route))
		Method = data[179]["method"]
		params = eval(data[179]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[179]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[179]["expect"],resp.json()["message"]))
