...
class BetRecordAE(unittest.TestCase):
	...
	def test_AE_bet_details_daily(self):
		...
		route = data[185]["route"]
		url = "".join(base.get_url(route))
		Method = data[185]["method"]
		params = eval(data[185]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[185]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[185]["expect"],resp.json()["message"]))
