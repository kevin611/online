...
class BetRecordAG(unittest.TestCase):
	...
	def test_AG_bet_details_daily(self):
		'''测试获取上月每天的AG电子投注详情'''
		route = data[177]["route"]
		url = "".join(base.get_url(route))
		Method = data[177]["method"]
		params = eval(data[177]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[177]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[177]["expect"],resp.json()["message"]))
