...
class BetRecordPT(unittest.TestCase):
	...
	def test_PT_bet_details_daily(self):
		'''测试获取上月每天的PT电子投注详情'''
		route = data[187]["route"]
		url = "".join(base.get_url(route))
		Method = data[187]["method"]
		params = eval(data[187]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[187]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[187]["expect"],resp.json()["message"]))
