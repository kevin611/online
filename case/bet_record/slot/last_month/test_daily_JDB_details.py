...
class BetRecordJDB(unittest.TestCase):
	...
	def test_JDB_bet_details_daily(self):
		'''测试获取上月每天的JDB电子投注详情'''
		route = data[181]["route"]
		url = "".join(base.get_url(route))
		Method = data[181]["method"]
		params = eval(data[181]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[181]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[181]["expect"],resp.json()["message"]))
