...
class BetRecordMW(unittest.TestCase):
	...
	def test_MW_bet_details_daily(self):
		'''测试获取上月每天的MW电子投注详情'''
		route = data[183]["route"]
		url = "".join(base.get_url(route))
		Method = data[183]["method"]
		params = eval(data[183]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[183]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[183]["expect"],resp.json()["message"]))
