...
class BetRecordFG(unittest.TestCase):
	...
	def test_FG_bet_details_daily(self):
		...
		route = data[178]["route"]
		url = "".join(base.get_url(route))
		Method = data[178]["method"]
		params = eval(data[178]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[178]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[178]["expect"],resp.json()["message"]))
