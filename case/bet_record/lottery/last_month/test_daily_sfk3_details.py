...
class BetRecordSfk3(unittest.TestCase):
	def setUp(self):
		self.log = logger.Log()
	def test_sfk3_bet_details_daily(self):
		'''测试获取上月每天的3分快3投注详情'''
		route = data[78]["route"]
		url = "".join(base.get_url(route))
		Method = data[78]["method"]
		params = eval(data[78]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[78]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[78]["expect"],resp.json()["message"]))
