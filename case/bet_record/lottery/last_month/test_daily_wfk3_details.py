...
class BetRecordWfk3(unittest.TestCase):
	'''测试获取上月每天的5分快3投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_wfk3_bet_details_daily(self):

		route = data[79]["route"]
		url = "".join(base.get_url(route))
		Method = data[79]["method"]
		params = eval(data[79]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[79]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[79]["expect"],resp.json()["message"]))
