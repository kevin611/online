...
class BetRecordXgsm(unittest.TestCase):
	'''测试获取上月每天的香港赛马投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_xgsm_bet_details_daily(self):

		route = data[71]["route"]
		url = "".join(base.get_url(route))
		Method = data[71]["method"]
		params = eval(data[71]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[71]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[71]["expect"],resp.json()["message"]))
