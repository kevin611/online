...
class BetRecordKsssc(unittest.TestCase):
	'''测试获取上月每天的快速时时彩投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_ksssc_bet_details_daily(self):

		route = data[82]["route"]
		url = "".join(base.get_url(route))
		Method = data[82]["method"]
		params = eval(data[82]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[82]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[82]["expect"],resp.json()["message"]))
