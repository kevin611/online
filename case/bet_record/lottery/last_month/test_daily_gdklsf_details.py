...
class BetRecordGdklsf(unittest.TestCase):
	'''测试获取上月每天的广东快乐十分投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_gdklsf_bet_details_daily(self):

		route = data[80]["route"]
		url = "".join(base.get_url(route))
		Method = data[80]["method"]
		params = eval(data[80]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[80]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[80]["expect"],resp.json()["message"]))
