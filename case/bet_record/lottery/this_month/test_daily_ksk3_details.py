...
class BetRecordKsk3(unittest.TestCase):
	def setUp(self):
		self.log = logger.Log()
	def test_ksk3_bet_details_daily(self):
		'''测试获取本月每天的快速快3投注详情'''
		route = data[56]["route"]
		url = "".join(base.get_url(route))
		Method = data[56]["method"]
		params = eval(data[56]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[56]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[56]["expect"],resp.json()["message"]))
