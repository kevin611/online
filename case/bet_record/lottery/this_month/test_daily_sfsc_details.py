...
class BetRecordSfsc(unittest.TestCase):
	'''测试获取本月每天的快乐赛车投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_sfsc_bet_details_daily(self):

		route = data[48]["route"]
		url = "".join(base.get_url(route))
		Method = data[48]["method"]
		params = eval(data[48]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[48]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[48]["expect"],resp.json()["message"]))
