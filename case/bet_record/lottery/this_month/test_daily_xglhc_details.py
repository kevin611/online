...
class BetRecordXglhc(unittest.TestCase):
	'''测试获取本月每天的香港六合彩投注详情'''
	def setUp(self):
		self.log = logger.Log()
	def test_xglhc_bet_details_daily(self):

		route = data[52]["route"]
		url = "".join(base.get_url(route))
		Method = data[52]["method"]
		params = eval(data[52]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[52]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[52]["expect"],resp.json()["message"]))
