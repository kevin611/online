...
class BetRecordKslhc(unittest.TestCase):
	def setUp(self):
		self.log = logger.Log()
	def test_kslhc_bet_details_daily(self):
		'''测试获取本月每天的快速六合彩投注详情'''
		route = data[53]["route"]
		url = "".join(base.get_url(route))
		Method = data[53]["method"]
		params = eval(data[53]["params"])
		...
		for d in res:
			...
			self.assertEqual(data[53]["expect"],resp.json()["message"],msg="失败原因为%s != %s" % ( data[53]["expect"],resp.json()["message"]))
