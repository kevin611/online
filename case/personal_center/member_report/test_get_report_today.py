...
class MemberReport(unittest.TestCase):
	...
	def test_memberreport_today(self):
		'''测试获取今日个人报表'''
		route = data[25]["route"]
		url = "".join(base.get_url(route))
		Method = data[25]["method"]
		json_data = eval(data[25]["data"])
		...
		self.assertIn(resp.json()["message"].replace("'\'","'\\'"),data[25]["expect"],msg="失败原因为%s != %s" % (resp.json()["message"].replace("'\'","'\\'"),data[25]["expect"]))
