...
class withdrawals(unittest.TestCase):
	...
	def test_withdrawals_thismonth(self):
		'''获取本月提现记录'''
		route = data[7]["route"]
		url = "".join(base.get_url(route))
		Method = data[7]["method"]
		headers = {"Authorization": self.token}
		params = eval(data[7]["params"])
		# 获取当前日期
		...
		self.assertIn(resp.json()["message"].replace("'\'","'\\'"),data[7]["expect"],msg="失败原因为%s not in %s"%(resp.json()["message"].replace("'\'","'\\'"), data[7]["expect"]))
