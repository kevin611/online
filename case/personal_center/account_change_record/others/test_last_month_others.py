...
class Others(unittest.TestCase):
	...
	def test_others_lastmonth(self):
		...
		Method = data[24]["method"]
		headers = {"Authorization": self.token}
		params = eval(data[24]["params"])
		# 获取当前日期
		...
		self.assertIn(resp.json()["message"].replace("'\'","'\\'"),data[24]["expect"],msg="失败原因为%s not in %s"%(resp.json()["message"].replace("'\'","'\\'"), data[24]["expect"]))
