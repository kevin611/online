...
class Others(unittest.TestCase):
	...
	def test_others_thisweek(self):
		route = data[21]["route"]
		url = "".join(base.get_url(route))
		Method = data[21]["method"]
		headers = {"Authorization": self.token}
		params = eval(data[21]["params"])
		...
		self.assertIn(resp.json()["message"].replace("'\'","'\\'"),data[21]["expect"],msg="失败原因为%s not in %s"%(resp.json()["message"].replace("'\'","'\\'"), data[21]["expect"]))
