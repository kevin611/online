...
class Deposit(unittest.TestCase):
	...
	def test_deposit_lastmonth(self):
		route = data[12]["route"]
		url = "".join(base.get_url(route))
		Method = data[12]["method"]
		headers = {"Authorization": self.token}
		params = eval(data[12]["params"])
		# 获取当前日期
		...
		self.assertIn(resp.json()["message"].replace("'\'","'\\'"),data[12]["expect"],msg="失败原因为%s not in %s"%(resp.json()["message"].replace("'\'","'\\'"), data[12]["expect"]))
